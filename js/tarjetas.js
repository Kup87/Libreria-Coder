let lightboxPrint = (e)=>{ //Fx para imprimir la tarjetita
    ...
    if (!array[14].replace(/\s/g, '').length) {//si están vacías
        ...
        imgTarjetita = (`<div class="bg-image hover-overlay ripple" id="imgCard" data-mdb-ripple-color="light">
        <img src="${imgTest}" id="imgCard_img" onerror="this.onerror=null;this.src='/assets/imagen_no_disponible.jpg';"/></div>`);
    }  
    ...
    $(".tarjeta").html(` 
        ...
        <div class="card-body">
            ...
            <h4 class="mb-2  text-center">${array[4]}</h4>
            <p class="card-text"><span>Compra: $${array[6]}</span> <span>Venta: $${array[8]}</span></p>
            <p class="card-text"><span>Estado:${array[12]}</span> <span>Stock${array[10]}</span></p>
        </div>
